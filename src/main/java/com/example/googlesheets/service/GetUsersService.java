...
e.printStackTrace();
	...
	private static String generateFileName(String functionName, String uuid) {
		
		return Constants.FN+"_"+functionName+"_"+uuid+".xlsx";

	}

	private static final String FILE_PATH = "users.xlsx";

	public static List<User> findUserByPhone(String phoneToSearch) {
	...
	public String getFunctionTypes() {
		...
		String jsonResponse = "";
		try (FileInputStream fis = new FileInputStream(new File(functionFile));
				Workbook workbook = StreamingReader.builder().rowCacheSize(100).bufferSize(4096).open(fis)) {
